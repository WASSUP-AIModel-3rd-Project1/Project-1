...
$$
d_{\text{record}}(A,B) = \left\Vert\left(w_i \cdot d_i(A,B)\right)_i \right\Vert_7\text{ , when }A,B\text{ are records, }d_i(A,B)\text{ are distance between }A\text{ and }B \\ \text{ on a property }i \in \{\text{city, race, sex, year}\}\text{, and weights }\left(w_\text{city}, w_\text{race}, w_\text{sex}, w_\text{year}\right)\text{ are }(0.4,0.5,0.5,0.1)\text{.}\\
